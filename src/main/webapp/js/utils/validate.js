...
$.extend($.fn.validatebox.defaults.rules,
				{
					...
					maxFloat:{
					 ...
					 safepass: {  
					        validator: function (value, param) {  
					        	var bool = /(?!^\\d+$)(?!^[a-zA-Z]+$)(?!^[_#@]+$).{8,}/.test(value);
					        	return bool;
					        },  
					        message: '密码必须为数字,字符,特殊符号组成，至少8位'  
					 },
					 floatLimit:{
						 
						 validator: function (value, param) {  
							 var bool = /^\d{1,5}(\.\d{1,5})?$/.test(value);
							 return bool;
						 },  
						 message: '只能输入5位整数，和小数5位'  
					 },
					 multiple : {  
			                    ...
			                    for (var i = 0; i < vtypes.length; i++) {  
			                        ...
			                        if (value && rule) {  
			                            ...
			                            if (!rule["validator"](value, parame)) {  
			                                returnFlag = false; 
			                                //只带一个参数
			                                var mes=rule.message;
			                                mes=mes.replace(new RegExp("\\{"+0+"\\}","g"),parame);  
			                                this.message = mes;
			                                break;  
			                            }  
			                        }  
